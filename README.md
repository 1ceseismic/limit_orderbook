...
at each pricelevel there is a std::list (non custom queue impl version) of orders listed for that price ;  this queue ensures time priority  i.e first-in, first-out
...
classDiagram
    ...
    class Order {
        std::map<uint64_t, optr>::iterator map_it;
        int client_id
        uint32_t token
        bool is_buy
        uint32_t quantity
    ...
    class GlobalState {
        ...
        map~uint32_t, Order*~ token_to_order
    }
